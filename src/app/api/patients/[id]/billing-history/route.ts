...
export async function GET(
  ...
  try {
    ...
    const serverUrl = `https://${projectId}.supabase.co/functions/v1/make-server-c89a26e4/patients/${encodeURIComponent(patientId)}/billing-history`;
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Failed to fetch billing history' }));
    ...
    return NextResponse.json(
      { error: 'Billing history service unavailable' },
      { status: 500 }
    );
  }
}